...
export default function OrderManagementScreen() {
    ...
    console.log("trạng thái đơn hàng:", orders.map(order => order.status));
    ...
    const renderOrderItem = ({ item }: { item: OrderType }) => (
        <View style={styles.orderCard}>
            ...
            <View style={styles.customerInfo}>
                ...
                <Text style={styles.customerName}>{item.customerName || item.userEmail}</Text>
            </View>

            <View style={styles.priceContainer}>
                <Text style={styles.totalLabel}>Tổng tiền</Text>
                <Text style={styles.totalAmount}>{formatPrice(item.totalAmount || item.finalPrice)}</Text>
            </View>
            ...
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    <View style={styles.modalHeader}>
                        ...
                        <Text style={styles.modalTitle}>Chi tiết đơn hàng #{selectedOrder?.id || selectedOrder?.orderId}</Text>
                        <TouchableOpacity
                    ...
                    <ScrollView style={styles.modalScrollView}>
                        <View style={styles.orderDetailsSection}>
                            ...
                            <View style={styles.detailRow}>
                                ...
                                <Text style={styles.detailValue}>#{selectedOrder?.id || selectedOrder?.orderId}</Text>
                            </View>
                            <View style={styles.detailRow}>
                                <Text style={styles.detailLabel}>Khách hàng:</Text>
                                <Text style={styles.detailValue}>{selectedOrder?.customerName || selectedOrder?.userEmail}</Text>
                            </View>
                            <View style={styles.detailRow}>
                                <Text style={styles.detailLabel}>Trạng thái:</Text>
                                <View style={[styles.statusBadgeSmall, { backgroundColor: getStatusColor(selectedOrder?.status) }]}>
                                    <Text style={styles.statusTextSmall}>{getStatusText(selectedOrder?.status)}</Text>
                                </View>
                            </View>
                            <View style={styles.detailRow}>
                                <Text style={styles.detailLabel}>Tổng tiền:</Text>
                                <Text style={styles.detailValuePrice}>{formatPrice(selectedOrder?.totalAmount || selectedOrder?.finalPrice)}</Text>
                            </View>
                        </View>

                        {Array.isArray(selectedOrder?.orderItemList) && selectedOrder.orderItemList.length > 0 ? (
                                    ...
                                    <View key={`${product.id}-${idx}`} style={styles.productItem}>
                                        ...
                                        <View style={styles.productInfo}>
                                            ...
                                            <View style={styles.productPriceRow}>
                                                ...
                                                <Text style={styles.productQuantity}>x{product.orderedQuantity}</Text>
                                            </View>
                                        </View>
                                    </View>
                                ))}
