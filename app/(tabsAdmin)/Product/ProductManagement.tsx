...
// Import useFocusEffect và useNavigation/useRoute từ React Navigation / Expo Router
import { useFocusEffect, useRouter } from 'expo-router';

// API fetch danh sách categories từ backend
...
const ProductCategoryScreen: React.FC = () => {
  ...
  // Hàm gọi API getCategories()
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const data = await getCategories();
      // Đảm bảo data là mảng string
  ...
  // useFocusEffect: mỗi khi màn được focus (lần đầu mount hoặc quay lại),
  // đều gọi fetchCategories() để cập nhật UI.
  useFocusEffect(
    useCallback(() => {
      fetchCategories();
    }, [])
  ...
  const renderCategoryItem = ({ item, index }: { item: string; index: number }) => (
    <TouchableOpacity
      ...
      // Khi nhấn vào 1 category, push sang màn ProductListScreen kèm param { category: item }
      onPress={() =>
        router.push({
